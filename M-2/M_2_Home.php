...
<div class="header">
    ...
    color: inherit;font-size : 25px" id="new-request-button" onclick="window.location.href = '/M-2/M_2_completedFiles.php'; ">Completed Files</button></h2>
            ...
            $sql = "SELECT COUNT(*) AS count FROM form_info WHERE (form_info.status != 'institute' or form_info.status != 'police' or form_info.status != 'officer' or form_info.status != 'rejected by institute' or form_info.status != 'rejected by SB' or form_info.status != 'rejected by institute' or form_info.status != 'requestor') and institute_email = '$instEmail'";
            $result = $conn->query($sql);
