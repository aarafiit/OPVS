...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($newPassword === $confirmedPassword) {
        ...
        $updatePasswordSql = "UPDATE police SET password = '$hashedNewPassword' WHERE official_mail = '$userEmail'";
        
        echo 'success';
        if ($conn->query($updatePasswordSql)) {
            echo "<script>alert('$userEmail');alert('$hashedNewPassword') ;window.location.href = 'M-4_P-2.php';</script>";
        } else {
