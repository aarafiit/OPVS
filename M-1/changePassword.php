...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($newPassword === $confirmedPassword && $tempPass === $tempPass2) {
        ...
        if ($conn->query($updatePasswordSql)) {
            echo "<script>alert('Password is changed successfully! ');window.location.href = 'M-1_P-2.php';</script>";
        } else {
            echo "Error updating password: " . $conn->error;
        }
    }
    else if($tempPass != $tempPass2){
        echo "<script>alert('The temporary password is not correct! ');</script>";
    }
...
<body>
    <div class="container">
        ...
        <label for="temporary_password">Enter the temporary Password sent to your email :</label>
